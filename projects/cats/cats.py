...
def choose(paragraphs, select, k):
    ...
    for i in paragraphs:
        if select(i):
            vaild_paragraphs.append(i)
    if k >= 0 and k < len(vaild_paragraphs):
    ...
    if typed == '':
        ...
        if len(typed_words) <= len(reference_words):
            ...
            for i in reference_words:
                if i == typed_words[reference_words.index(i)]:
                    count += 1
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        if i == user_word:
            return user_word
        li.append(diff_function(user_word, i, limit))
    if min(li) <= limit:
        return valid_words[li.index(min(li))]
    else:
        return user_word
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    li1 = []
    li = []
    for j in times_per_player:
        for i in range(len(j)-1):
            if i <= len(words) - 1:
                numberclear = j[i + 1] - j[i]
                li1.append(numberclear)
            else:
                numberclear = j[-1] - j[i]
                li1.append(numberclear)
        li.append(li1)
        li1 = []
    return game(words, li)
    # END PROBLEM 9
